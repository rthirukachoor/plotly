...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    let metaDataFilteredData=metaData.filter((metaDataRow) =>
       metaDataRow.id == sample);
     
    
    console.log(metaDataFilteredData);
    


    //  5. Create a variable that holds the first sample in the array.
    let sampleFilteredDataFirst=sampleFilteredData[0];  
    let metaDataFilteredDataFirst = metaDataFilteredData[0];
    ...
    let washing_frequency = parseFloat(metaDataFilteredDataFirst.wfreq);
